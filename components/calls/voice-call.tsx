...
export function VoiceCall({ mateId, mateName, onCallEnd }: VoiceCallProps) {
  ...
  useEffect(() => {
    ...
    if (localStream && localAudioRef.current) {
      ...
      localAudioRef.current.muted = true; // Must be muted to prevent echo
    ...
    // CRITICAL: Poll for remote stream
  ...
  if (isMinimized) {
    return (
      <div className="fixed bottom-4 left-4 z-[9999] glass px-4 py-2 rounded-lg flex items-center gap-3 font-mono border border-purple-400/50">
        ...
        {/* CRITICAL: Keep audio elements alive even when minimized */}
  ...
  return (
    ...
    >
      {/* CRITICAL: Audio elements must always be mounted */}
      <audio ref={localAudioRef} autoPlay muted hidden />
      <audio ref={remoteAudioRef} autoPlay hidden />
      ...
      <div className="h-[calc(100%-40px)] flex flex-col items-center justify-center bg-gradient-to-br from-black via-purple-950/20 to-pink-950/20 relative overflow-hidden">
        ...
        {/* Network Metrics HUD */}
        <div className="absolute top-16 left-4 glass px-3 py-2 rounded border border-cyan-400/50 font-mono text-xs space-y-1">
