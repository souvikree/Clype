...
export function GoogleLoginComponent() {
    ...
    const setupDesktopOAuth = async () => {
      ...
      cleanupListener = await listenForOAuthCallback(async (url: string) => {
        try {
          ...
          if (!dataParam) {
            console.error('❌ No data parameter in deep link')
            setDebugInfo('Error: Missing data in callback')
            setIsLoading(false)
            return
          }
          ...
          // CRITICAL: Set auth state synchronously
          setUser({
            id: userData.userId,
            email: userData.email,
          ...
          setDebugInfo('Login failed: ' + (error instanceof Error ? error.message : 'Unknown error'))
          setIsLoading(false);
        }
      });
  ...
  }, [isDesktop, setUser, setToken, setIsLoading, router])
  ...
  const handleDesktopLogin = async () => {
    try {
      ...
      console.log('🚀 DESKTOP GOOGLE LOGIN - START')
      
      // ✅ FIX #1: Set loading state FIRST
      setIsLoading(true)
      setDebugInfo('Opening browser for Google login...')
      ...
      console.log('➡️ URL:', authUrl)

      // ✅ FIX #2: Import from correct package and call correctly
      const { open } = await import('@tauri-apps/plugin-shell')
      
      console.log('📦 Shell plugin loaded, calling open()...')
      await open(authUrl)

      console.log('✅ Browser opened successfully')
      setDebugInfo('Complete login in browser, then return here...')

      // ✅ FIX #3: Don't clear loading state here - wait for deep link callback
      // setIsLoading(false)  // ❌ DON'T DO THIS
